...
REM simple-metricd - Build Script
...
:build_project
call :print_status "Building simple-metricd..."
...
:install_project
if "%INSTALL%"=="true" (
    call :print_status "Installing simple-metricd..."
    cd /d "%BUILD_DIR%"
...
if "%ENABLE_STATIC_LINKING%"=="ON" (
    ...
    set "STATIC_DIR=%DIST_DIR%\simple-metricd-0.1.0-static-windows"
    if not exist "%STATIC_DIR%" mkdir "%STATIC_DIR%"
    
    REM Copy binary and files
    copy "%BUILD_DIR%\simple-metricd.exe" "%STATIC_DIR%\"
    ...
    cd /d "%DIST_DIR%"
    powershell -Command "Compress-Archive -Path 'simple-metricd-0.1.0-static-windows' -DestinationPath 'simple-metricd-0.1.0-static-windows.zip' -Force"
    rmdir /s /q "simple-metricd-0.1.0-static-windows"
    
    call :print_success "Static binary package created: simple-metricd-0.1.0-static-windows.zip"
)
goto :eof

REM Function to show usage
:show_usage
echo simple-metricd - Windows Build Script
echo.
...
REM Main execution
:main
call :print_status "Starting simple-metricd build process..."
...
call :print_success "simple-metricd build process completed successfully!"
goto :end
