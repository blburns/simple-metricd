...
# simple-metricd macOS Build Script
# This script builds the simple-metricd application for macOS
...
create_static_package() {
    if [ "$ENABLE_STATIC_LINKING" = "ON" ]; then
        ...
        STATIC_DIR="$DIST_DIR/simple-metricd-$VERSION-static-macos"
        mkdir -p "$STATIC_DIR"
        
        # Copy binary and files
        cp "$BUILD_DIR/simple-metricd" "$STATIC_DIR/"
        ...
        # Create tarball
        cd "$DIST_DIR"
        tar -czf "simple-metricd-$VERSION-static-macos.tar.gz" "simple-metricd-$VERSION-static-macos"
        rm -rf "simple-metricd-$VERSION-static-macos"
        
        print_success "Static binary package created: simple-metricd-$VERSION-static-macos.tar.gz"
    fi
}
