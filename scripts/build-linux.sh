#!/bin/bash
# Generic Linux build script for simple-metricd
...
build_project() {
    print_status "Building simple-metricd..."
...
install_project() {
    if [ "$INSTALL" = "true" ]; then
        print_status "Installing simple-metricd..."
        cd "$BUILD_DIR"
        
        sudo make install
        
        # Test installation
        if command_exists simple-metricd; then
...
create_service() {
    if [ "$SERVICE" = "true" ]; then
        ...
        sudo cp "$PROJECT_ROOT/deployment/systemd/simple-metricd.service" /etc/systemd/system/simple-metricd.service

        sudo getent group simple-metricd >/dev/null 2>&1 || sudo groupadd --system simple-metricd
        sudo getent passwd simple-metricd >/dev/null 2>&1 || \
            sudo useradd --system --home-dir /var/lib/simple-metricd --no-create-home \
                --shell /usr/sbin/nologin --gid simple-metricd simple-metricd
        ...
        print_status "Copy a production template to /etc/simple-metricd/simple-metricd.conf, then 'sudo systemctl start simple-metricd'"
    fi
}
...
show_usage() {
    cat << EOF
simple-metricd - Linux Build Script
...
# Main execution
main() {
    print_status "Starting simple-metricd build process..."
    ...
    print_success "simple-metricd build process completed successfully!"
}
